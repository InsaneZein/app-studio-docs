...
var calculateSelected = function (selectedTags) { return function (type, groupKey, value, checked) {
    ...
    var itemKeys = type === groupType_1.default.treeView ? children.map(function (item) { return item.id; }) : [value];
    if (activeGroup) {
        var result_1 = selectedTags;
        itemKeys.map(function (itemKey) {
            var _a, _b, _c, _d;
            var activeGroup = result_1[groupKey];
            if (type !== groupType_1.default.radio && (activeGroup[itemKey] instanceof Object ? activeGroup[itemKey].isSelected : Boolean(activeGroup[itemKey]))) {
